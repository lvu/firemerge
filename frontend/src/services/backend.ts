...
export async function getTransactions(accountId: number): Promise<Transaction[] | null> {
    const data = await apiFetch<Transaction[]>(`/api/transactions`, { account_id: accountId.toString() });
    if (data === null) {
        return null;
    }
    return data.map(tr => ({
...
export async function uploadTransactions(file: File, timezone: string): Promise<void> {
    ...
    await apiFetch<void>(`/api/upload`, { timezone }, {
        method: 'POST',
        body: formData,
    });
}
...
export async function searchDescriptions(accountId: number, query: string): Promise<string[]> {
    return (await apiFetch<string[]>(`/api/descriptions`, { account_id: accountId.toString(), query }))!;
}